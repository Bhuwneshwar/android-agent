...
const toolsDeclaration = [
    // only for macrodroid can be used in android agent
    {
        ...
        description:
            "Execute a raw Linux command in the current environment and return its output. (e.g., Termux) or Linux-based systems.",
        parameters: {
            type: Type.OBJECT,
            properties: {
                command: {
                    type: Type.STRING,
                    description:
                        "Linux shell command to execute (e.g., 'ls -la', 'df -h' npm, node, and more)"
                }
            },
            required: ["command"]
        }
...

    {
        name: "runNodeJsCode",
        description:
            "Dynamically write and execute JavaScript code using Node.js. Code is saved to a temporary file and executed with 'node'. Supports es module import statements and we can use any node modules like 'node:fs' more .",
        parameters: {
            type: Type.OBJECT,
            properties: {
                code: {
                    type: Type.STRING,
                    description:
                        "The Node.js code snippet to run (e.g., 'console.log(2 + 2);' Supports es module import statements and we can use any node modules like 'node:fs' more .)"
                }
            },
            required: ["code"]
        }
...

    {
        name: "GenNewQuestion",
        description:
            "Generate a new full-stack web developer quiz question with options and an explanation.",
        parameters: {
            type: Type.OBJECT,
            properties: {
                question: {
                    type: Type.STRING,
                    description: "A full-stack developer question."
                },
                option1: {
                    type: Type.STRING,
                ...
                correct_option: {
                    type: Type.STRING,
                    description:
                        "The key of the correct option (e.g., 'option1', 'option2')."
                },
                explanation: {
                    type: Type.STRING,
                    description:
    ...
    },
    {
        name: "searchContacts",
        description:
            "search contact by name or number. ✔ Partial name search,✔ Multi-word name search ✔ Partial phone number search ✔ Case-insensitive ✔Safe & simple",
        parameters: {
            type: Type.OBJECT,
            properties: {
                query: {
                    type: Type.STRING,
                    description: "name or number for search"
                }
            },
            required: ["query"]
        }
    ...
    {
        name: "addImportantNote",
        descript: "add important note for never lose them",
        parameters: {
            type: Type.OBJECT,
            properties: {
                note: {
                    type: Type.STRING,
                    description: "note to add on notes book"
                }
            },
            required: ["note"]
        }
    ...
    {
        name: "getAllImportantNotes",
        descript: "get all important notes"
    },
    {
        name: "updateAllImportantNotes",
        descript: "update important notes for correct notes",
        parameters: {
            type: Type.OBJECT,
            properties: {
                notes: {
                    type: Type.STRING,
                    description: "notes to update on notes book"
                }
            },
            required: ["notes"]
        }
    ...
    {
        name: "askAnyThingToAi",
        descript:
            "this is another ai if you can't answer any user prompt to forward to this ai",
        parameters: {
            type: Type.OBJECT,
            properties: {
                prompt: {
        ...
        parameters: {
            ...
            properties: {
                ...
                message: {
                    type: Type.STRING,
                    description:
                        "message to send when time is up e.g. light off kar do"
                }
            },
        ...
        parameters: {
            ...
            properties: {
                number: {
                    type: Type.STRING,
                    description:
                        "number to send require number with country code"
                },
                message: {
                    type: Type.STRING,
    ...
    {
        name: "clickOnTextContent",
        description: "click on text content in screen showing",
        parameters: {
            type: Type.OBJECT,
            properties: {
                text: {
                    type: Type.STRING,
                    description: "text content in display"
                }
            },
            required: ["text"]
        }
    ...
    {
        name: "clickById",
        description: "click by id in display showing",
        parameters: {
            type: Type.OBJECT,
            properties: {
                id: {
                    type: Type.STRING,
                    description: "id in display showing"
                }
            },
            required: ["id"]
        }
    ...
    {
        name: "runShellScriptInAndroidEnv",
        description:
            "run shell script in android environment. support root,adb access",
        parameters: {
            type: Type.OBJECT,
            properties: {
                script: {
